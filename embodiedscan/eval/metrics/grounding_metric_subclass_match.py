...
class GroundingMetricMod(BaseMetric):
    ...
    def ground_eval_subset(self, gt_annos, det_annos, logger=None, prefix=''):
        ...
        # these lists records for each gt box, NOT for each sample
        ious = [[] for _ in self.iou_thr] # each gt box has a best matching iou for each iou ths (because score has priority)
        ...
        for sample_idx in range(num_samples):
            ...
            for iou_idx, ths in self.iou_thr:
                cost_mat = np.zeros_like(iou_mat)
                cost_mat = iou_mat < ths # higher iou -> lower cost
                cost_mat -= target_scores[:, np.newaxis]
                matched_pred_inds, matched_gt_inds = linear_sum_assignment(cost_mat)
                matched_ious = iou_mat[matched_pred_inds, matched_gt_inds]
                for i, score in enumerate(target_scores):
                    confidences.append(score)
                    if i in matched_pred_inds:
                        idx = list(matched_pred_inds).index(i)
                        ious[iou_idx].append(matched_ious[idx])
                    else:
                        ious[iou_idx].append(0)
        
        confidences = np.array(confidences)
        sorted_inds = np.argsort(-confidences)
        ious = np.array(ious)[:, sorted_inds]
        tp_thr = {}
        fp_thr = {}
        for idx, thr in enumerate(self.iou_thr):
            tp_thr[f'{prefix}@{thr}'] = np.zeros(len(confidences))
            fp_thr[f'{prefix}@{thr}'] = np.zeros(len(confidences))
            thr_mask = ious[idx] >= thr  
            tp_thr[f'{prefix}@{thr}'][thr_mask] = 1.0  
            fp_thr[f'{prefix}@{thr}'][~thr_mask] = 1.0  
